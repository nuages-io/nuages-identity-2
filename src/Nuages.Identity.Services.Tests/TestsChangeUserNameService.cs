...
public class TestsChangeUserNameService
{
    ...
    public async Task ShouldChangeUSerNameWithErrorsUsernameNotChanged()
    {
        ...
        var identityStuff = MockHelpers.MockIdentityStuff(user);
        identityStuff.UserStore.Setup(u => u.FindByNameAsync(user.UserName, It.IsAny<CancellationToken>()))
            .ReturnsAsync(() => user);
    ...
    public async Task ShouldChangeUSerNameWithErrorsUsernameAlreadyUsed()
    {
        ...
        var identityStuff = MockHelpers.MockIdentityStuff(user);
        identityStuff.UserStore.Setup(u => u.FindByNameAsync("EXISTING", It.IsAny<CancellationToken>())).ReturnsAsync(
